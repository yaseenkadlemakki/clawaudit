...
async def test_wrong_bearer_token_returns_401(_full_setup):
    ...
    assert r.status_code == 401, f"Expected 401, got {r.status_code}"
    body = r.json()
    assert "detail" in body or "error" in body or r.status_code == 401
...
def test_max_body_size_middleware_unit():
    ...
    from backend.main import MaxBodySizeMiddleware  # noqa: PLC0415
...
def test_scan_for_proposals_skips_openclaw_system_skills():
    """scan_for_proposals skips skills whose resolved path is under the openclaw prefix."""
    from sentinel.remediation.engine import RemediationEngine  # noqa: PLC0415

    # coding-agent exists on this machine under the protected prefix
    coding_agent = Path("/opt/homebrew/lib/node_modules/openclaw/skills/coding-agent")
    if not coding_agent.is_dir():
        pytest.skip("coding-agent skill not found at expected path")
...
async def test_no_token_in_findings_response(_full_setup):
    """GET /api/v1/findings response body must not expose the raw API token."""
    from backend.main import app as _app  # noqa: PLC0415
    ...
    assert TEST_TOKEN not in r.text, (
        "API token must not appear in the /api/v1/findings response body"
    )


async def test_no_token_in_skills_response(_full_setup):
    """GET /api/v1/skills response body must not expose the raw API token."""
    from backend.main import app as _app  # noqa: PLC0415
    ...
    assert r.status_code == 200, f"Expected 200, got {r.status_code}"
    assert TEST_TOKEN not in r.text, (
        "API token must not appear in the /api/v1/skills response body"
    )
