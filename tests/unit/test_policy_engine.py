...
def test_pol_001_discord_group_policy():
    pol_dir = _make_policy_dir([{
        "id": "POL-001", "domain": "config", "check": "discord.groupPolicy",
        "condition": "not_equals", "value": "allowlist", "severity": "CRITICAL",
        "action": "ALERT", "message": "test"
    }])
    ...
    finding = Finding(
        ...
        evidence="groupPolicy=open", location="openclaw.json → discord.groupPolicy",
        remediation="", run_id="r1"
    )
    # Use check field matching
    event = Event(
        source="config_collector", event_type="config_audit_fail", severity="CRITICAL",
        entity="openclaw.json", evidence="check_id=CONF-01 groupPolicy=open",
        action_taken="ALLOW"
    )
...
def test_pol_007_runaway_agent():
    pol_dir = _make_policy_dir([{
        "id": "POL-007", "domain": "runtime", "check": "tool_calls_per_minute",
        "condition": "gt", "value": "30", "severity": "HIGH",
        "action": "ALERT", "message": "runaway"
    }])
    engine = PolicyEngine(pol_dir)

    event = Event(
        source="session_collector", event_type="runaway_agent", severity="HIGH",
        entity="session-abc", evidence="tool_calls_per_minute=45 threshold=30",
        action_taken="ALLOW"
    )
    decision = engine.evaluate(event)
...
def test_pol_008_unauthorized_cron():
    pol_dir = _make_policy_dir([{
        "id": "POL-008", "domain": "runtime", "check": "event_type",
        "condition": "equals", "value": "unauthorized_cron", "severity": "HIGH",
        "action": "ALERT", "message": "cron"
    }])
    engine = PolicyEngine(pol_dir)

    event = Event(
        source="cron_collector", event_type="unauthorized_cron", severity="HIGH",
        entity="cron-xyz", evidence="new_cron_id=cron-xyz", action_taken="ALLOW"
    )
    decision = engine.evaluate(event)
    assert decision.action == "ALERT"
...
def test_pol_009_new_skill():
    pol_dir = _make_policy_dir([{
        "id": "POL-009", "domain": "skills", "check": "event_type",
        "condition": "equals", "value": "new_skill", "severity": "MEDIUM",
        "action": "ALERT", "message": "new skill"
    }])
    engine = PolicyEngine(pol_dir)

    event = Event(
        source="skill_collector", event_type="new_skill", severity="MEDIUM",
        entity="evil-skill", evidence="path=/tmp/evil-skill/SKILL.md", action_taken="ALLOW"
    )
    decision = engine.evaluate(event)
    assert decision.action == "ALERT"
...
def test_pol_010_config_drift():
    pol_dir = _make_policy_dir([{
        ...
        "condition": "equals", "value": "config_drift", "severity": "HIGH",
        "action": "ALERT", "message": "drift"
    }])
    engine = PolicyEngine(pol_dir)

    event = Event(
        source="config_collector", event_type="config_drift", severity="HIGH",
        entity="openclaw.json", evidence="config_hash_changed=abc123", action_taken="ALLOW"
    )
    decision = engine.evaluate(event)
    assert decision.action == "ALERT"
...
def test_allow_when_no_rules_match():
    ...
    event = Event(
        source="config_collector", event_type="config_ok", severity="INFO",
        entity="openclaw.json", evidence="", action_taken="ALLOW"
    )
    decision = engine.evaluate(event)
...
def test_highest_action_wins():
    pol_dir = _make_policy_dir([
        {"id": "R1", "domain": "runtime", "check": "event_type", "condition": "equals",
         "value": "test_event", "severity": "LOW", "action": "WARN", "message": "warn"},
        {"id": "R2", "domain": "runtime", "check": "event_type", "condition": "equals",
         "value": "test_event", "severity": "HIGH", "action": "BLOCK", "message": "block"},
    ])
    engine = PolicyEngine(pol_dir)

    event = Event(
        source="test", event_type="test_event", severity="HIGH",
        entity="test", evidence="", action_taken="ALLOW"
    )
    decision = engine.evaluate(event)
