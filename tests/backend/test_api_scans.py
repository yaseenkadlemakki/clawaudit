...
@pytest.mark.asyncio
async def test_stop_scan_running(client):
    """Stop a running scan — status transitions to stopping."""
    r = await client.post("/api/v1/scans", json={"triggered_by": "test"})
    assert r.status_code == 201
    scan_id = r.json()["id"]

    r2 = await client.delete(f"/api/v1/scans/{scan_id}/stop")
    assert r2.status_code == 200
    assert r2.json()["status"] in ("stopping", "completed")
